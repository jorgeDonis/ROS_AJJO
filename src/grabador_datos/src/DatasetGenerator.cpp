...
void DatasetGenerator::add_to_dataset(KeyboardAction user_input, cv::Mat const &img)
{
    std::string filename = to_str(user_input) + "_" + std::to_string(num_images) + ".jpg";
    printf("adding image number %lu with name %s to dataset\n", num_images, filename.c_str());
